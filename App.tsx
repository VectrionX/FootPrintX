...
const App: React.FC = () => {
  ...
  const renderModule = () => <div className="space-y-6"><Disclaimer /><section aria-label="Investigation context summary" className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-4 text-sm text-slate-200"><div className="flex flex-wrap items-center justify-between gap-2"><strong className="text-emerald-200">Investigation context</strong><span className={ready ? 'text-emerald-300' : 'text-amber-300'}>{ready ? 'Authorization and scope confirmed' : 'Complete authorization and scope before preparing queries'}</span></div><dl className="mt-3 grid gap-2 sm:grid-cols-3"><div><dt className="text-xs text-slate-500">Case</dt><dd className="truncate">{context.caseName || 'Not provided'}</dd></div><div><dt className="text-xs text-slate-500">Authorization</dt><dd className="truncate">{context.authorization || 'Not provided'}</dd></div><div><dt className="text-xs text-slate-500">Approved scope</dt><dd className="truncate">{context.scope || 'Not provided'}</dd></div></dl></section><section className="rounded-3xl border border-white/10 bg-[#111827] p-6"><div className="flex items-start gap-3"><Search className="mt-1 text-blue-400" aria-hidden="true" /><div><h1 className="text-2xl font-bold text-white">{active.label}</h1><p className="mt-2 text-sm text-slate-400">{active.description}</p></div></div><div className="mt-6">{queryInput()}</div>{active.hasInput && !active.valid && <p role="alert" className="mt-3 flex items-center gap-2 text-sm text-amber-300"><AlertCircle className="h-4 w-4" />Enter a valid value before previews can be prepared.</p>}{active.valid && ready && <p className="mt-3 flex items-center gap-2 text-sm text-emerald-300"><CheckCircle2 className="h-4 w-4" />Local query previews are ready for review; nothing has been sent to a provider.</p>}</section>{active.categories.length ? <div className="space-y-4">{active.categories.map((category) => <DorkSection key={category.id} category={category} onRequestOpen={setPendingDork} />)}</div> : <EmptyState ready={ready} hasInput={active.hasInput} />}</div>;
